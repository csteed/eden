	...
	public void run() {
		...
		for (int iaxis = 0; iaxis < axisList.size(); iaxis++) {
			...
			if (this.showFrequencyInfo) {
				...
				// g2.fill(axis.frequencyDisplayRectangle);
				...
				double maxBinHeight = (double) axis.axisHeight
						/ (double) freqData.length;
				// int maxBinWidth = (axis.axisBarRectangle.width/2) - 2;
			...
			if (showAxesAsBars) {
				...
				int x1 = axis.axisBarRectangle.x + axis.axisBarRectangle.width
						+ 4;
				g2.drawLine(x0, axis.axisBarRectangle.y, x1,
						axis.axisBarRectangle.y);
				g2.drawLine(axis.xPosition - 2, axis.bottomPosition,
						axis.xPosition + 2, axis.bottomPosition);
				...
				if (dispersionDisplayMode == PCPanel.MEDIAN_DISPERSION_BOX_MODE) {
					...
					// draw the mean line
					g2.drawLine(
							axis.standardDeviationRangeRectangle.x,
							axis.meanPosition,
							axis.standardDeviationRangeRectangle.x
				...
				// draw axis as a line
				g2.setColor(AXIS_BAR_LINE_COLOR);
				g2.drawLine(axis.xPosition, axis.topPosition, axis.xPosition,
						axis.bottomPosition);
				g2.drawLine(axis.xPosition - 2, axis.topPosition,
						axis.xPosition + 2, axis.topPosition);
				g2.drawLine(axis.xPosition - 2, axis.bottomPosition,
						axis.xPosition + 2, axis.bottomPosition);
			}
